...
Los dashboards construidos en Oracle Analytics Cloud incluyen:

| Dashboard | Descripción |
|---|---|
| **Resumen Ejecutivo** | KPIs globales: ingresos, margen, clientes activos |
| **Ventas por Canal** | Comparativa de canales de distribución (barras, donut) |
| **Análisis de Productos** | Top 10 productos, categorías, tendencias temporales |
| **Perfil de Clientes** | Segmentación demográfica, nivel de ingreso, ocupación |
| **Mapa Argentina** | Mapa coroplético de ventas por provincia |
| **Efecto Promociones** | Impacto de cada tipo de promoción en el volumen vendido |
| **Evolución Temporal** | Serie de tiempo mensual y trimestral de ventas |
...
📁 oac-dw-argentina/
├── 📁 sql/
│   ├── 01_create_schema.sql       # Creación del esquema DW_SH
│   ├── 02_create_dimensions.sql   # DDL de todas las dimensiones
│   ├── 03_create_fact.sql         # DDL de FACT_VENTAS
│   └── 04_etl_load.sql            # Scripts de carga desde SH
├── 📁 geojson/
│   └── provincias_argentina.geojson  # Capa geoespacial de Argentina
├── 📁 oac/
│   ├── modelo_semantico.rpd       # Modelo semántico OAC (si aplica)
│   └── dashboards_export/         # Exportación de workbooks OAC
├── 📁 docs/
│   ├── arquitectura.png           # Diagrama de arquitectura
│   └── modelo_dimensional.png     # Diagrama del star schema
└── README.md
```
...
- Instancia de **Oracle Autonomous Database** (ADW o ATP) aprovisionada
- Instancia de **Oracle Analytics Cloud** aprovisionada
- Esquema **SH** disponible en la base de datos (incluido por defecto en Oracle DB)

### Pasos

```bash
# 1. Clonar el repositorio
git clone https://github.com/tu-usuario/oac-dw-argentina.git
cd oac-dw-argentina

# 2. Conectarse a la Autonomous Database con SQL*Plus o SQL Developer
# y ejecutar los scripts en orden:
sqlplus ADMIN@<tu_wallet_connection>
@sql/01_create_schema.sql
@sql/02_create_dimensions.sql
@sql/03_create_fact.sql
@sql/04_etl_load.sql

# 3. En OAC, subir el GeoJSON de provincias:
#    Consola OAC → Mapas → Capas de Mapa → Agregar Capa
#    Seleccionar: geojson/provincias_argentina.geojson

# 4. Crear la conexión en OAC hacia la Autonomous Database
#    usando el wallet descargado desde OCI.

# 5. Importar o recrear los datasets y dashboards desde oac/
```
